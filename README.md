...
* [`src/xviii_lization/styles.cljs`](src/xviii_lization/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/xviii_lization/`](src/xviii_lization/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/xviii_lization/core.cljs): contains the SPA entry point, `init`
* [`test/xviii_lization/`](test/xviii_lization/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `xviii-lization` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/xviii_lization/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns xviii-lization.example
  (:require [xviii-lization.config :as config])
